...
export function ProviderForm({
  ...
  const presetEntries = useMemo(() => {
    if (appId === "codex") {
      return ([] as any[]).map<PresetEntry>((preset, index) => ({
        id: `codex-${index}`,
        preset,
      }));
  ...
  // API Key link disabled (presets removed) - all custom providers
  const shouldShowClaudeApiKeyLink = false;
  const claudeWebsiteUrl = "";
  const isClaudePartner = false;
  const claudePartnerPromotionKey = "";

  const shouldShowCodexApiKeyLink = false;
  const codexWebsiteUrl = "";
  const isCodexPartner = false;
  const codexPartnerPromotionKey = "";

  const shouldShowGeminiApiKeyLink = false;
  const geminiWebsiteUrl = "";
  const isGeminiPartner = false;
  const geminiPartnerPromotionKey = "";

  const shouldShowOpencodeApiKeyLink = false;
  const opencodeWebsiteUrl = "";
  const isOpencodePartner = false;
  const opencodePartnerPromotionKey = "";
