...
SC-Tools v3.9.0 是 v3.9 测试版序列（`3.9.0-1`、`3.9.0-2`、`3.9.0-3`）的稳定版。
...
- 为降低导入风险，SQL 导入被限制为仅允许导入 SC-Tools 自己导出的备份。
- Proxy 接管会修改 CLI 的 live 配置；SC-Tools 会在重定向前自动备份 live 配置。如需回退，可关闭接管/停止代理，并在必要时从备份恢复。
...
访问 [Releases](https://github.com/farion1231/sc-tools/releases/latest) 下载对应版本。
...
| 文件                                    | 说明                                |
| --------------------------------------- | ----------------------------------- |
| `SC-Tools-v3.9.0-Windows.msi`          | **推荐** - MSI 安装包，支持自动更新 |
| `SC-Tools-v3.9.0-Windows-Portable.zip` | 便携版，解压即用，不写入注册表      |

### macOS

| 文件                            | 说明                                                      |
| ------------------------------- | --------------------------------------------------------- |
| `SC-Tools-v3.9.0-macOS.zip`    | **推荐** - 解压后拖入 Applications 即可，Universal Binary |
| `SC-Tools-v3.9.0-macOS.tar.gz` | 用于 Homebrew 安装和自动更新                              |
...
| 发行版                                  | 推荐格式    | 安装方式                                                               |
| --------------------------------------- | ----------- | ---------------------------------------------------------------------- |
| Ubuntu / Debian / Linux Mint / Pop!\_OS | `.deb`      | `sudo dpkg -i SC-Tools-*.deb` 或 `sudo apt install ./SC-Tools-*.deb` |
| Fedora / RHEL / CentOS / Rocky Linux    | `.rpm`      | `sudo rpm -i SC-Tools-*.rpm` 或 `sudo dnf install ./SC-Tools-*.rpm`  |
| openSUSE                                | `.rpm`      | `sudo zypper install ./SC-Tools-*.rpm`                                |
| Arch Linux / Manjaro                    | `.AppImage` | 添加执行权限后直接运行，或使用 AUR                                     |
| 其他发行版 / 不确定                     | `.AppImage` | `chmod +x SC-Tools-*.AppImage && ./SC-Tools-*.AppImage`              |
| 沙箱隔离需求                            | `.flatpak`  | `flatpak install SC-Tools-*.flatpak`                                  |
