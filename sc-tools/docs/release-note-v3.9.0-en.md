...
SC-Tools v3.9.0 is the stable release of the v3.9 beta series (`3.9.0-1`, `3.9.0-2`, `3.9.0-3`).
...
- SQL import is restricted to SC-Tools exported backups to reduce the risk of importing unsafe or incompatible SQL dumps.
- Proxy takeover modifies CLI live configs; SC-Tools will back up the live config before redirecting it to the local proxy. If you want to revert, disable takeover/stop the proxy and restore from the backup when needed.
...
Visit [Releases](https://github.com/farion1231/sc-tools/releases/latest) to download the appropriate version.
...
| File                                    | Description                                        |
| --------------------------------------- | -------------------------------------------------- |
| `SC-Tools-v3.9.0-Windows.msi`          | **Recommended** - MSI installer with auto-update support |
| `SC-Tools-v3.9.0-Windows-Portable.zip` | Portable version, no installation required         |

### macOS

| File                            | Description                                                       |
| ------------------------------- | ----------------------------------------------------------------- |
| `SC-Tools-v3.9.0-macOS.zip`    | **Recommended** - Extract and drag to Applications, Universal Binary |
| `SC-Tools-v3.9.0-macOS.tar.gz` | For Homebrew installation and auto-update                         |
...
| --------------------------------------- | ------------------ | ---------------------------------------------------------------------- |
| Ubuntu / Debian / Linux Mint / Pop!\_OS | `.deb`             | `sudo dpkg -i SC-Tools-*.deb` or `sudo apt install ./SC-Tools-*.deb` |
| Fedora / RHEL / CentOS / Rocky Linux    | `.rpm`             | `sudo rpm -i SC-Tools-*.rpm` or `sudo dnf install ./SC-Tools-*.rpm`  |
| openSUSE                                | `.rpm`             | `sudo zypper install ./SC-Tools-*.rpm`                                |
| Arch Linux / Manjaro                    | `.AppImage`        | Make executable and run directly, or use AUR                           |
| Other distros / Unsure                  | `.AppImage`        | `chmod +x SC-Tools-*.AppImage && ./SC-Tools-*.AppImage`              |
| Sandboxed installation                  | `.flatpak`         | `flatpak install SC-Tools-*.flatpak`                                  |
